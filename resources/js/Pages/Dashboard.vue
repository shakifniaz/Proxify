...
const statusBadge = {
    ...
    Assigned: 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30',
};
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/50 p-5">
                    <div>
                        ...
                        <span
                            class="mt-3 inline-flex rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-400"
                        >
                            {{ routineSummary.termLabel }}
                        </span>
                    </div>
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-emerald-500/15">
                        <CalendarDays class="h-6 w-6 text-emerald-400" />
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/50 p-5">
                    ...
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-rose-500/15">
                        <Repeat class="h-6 w-6 text-rose-400" />
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-slate-800 bg-slate-900/50">
                <div class="flex items-center justify-between border-b border-slate-800 px-5 py-3">
                    ...
                    <a href="#" class="flex items-center gap-1 text-sm font-medium text-emerald-400 hover:text-emerald-300">
                ...
                <div class="divide-y divide-slate-800">
                    ...
                    >
                        <span class="h-2.5 w-2.5 rounded-full" :class="colorClasses[stat.color].dot"></span>
                        <p class="flex-1 text-sm font-medium text-slate-200">{{ stat.label }}</p>
                        <p class="text-sm text-slate-500">{{ stat.sub }}</p>
                        <p class="w-10 text-right text-sm font-semibold" :class="colorClasses[stat.color].text">
                            {{ stat.value }}
                        </p>
                    </div>
                </div>
            ...
            <!-- Today at a glance + month stats / live activity -->
                ...
                <div class="rounded-xl border border-slate-800 bg-slate-900/50 p-6 lg:col-span-2">
                    ...
                    <div class="mt-6 space-y-3">
                        <div class="flex items-center justify-between rounded-lg bg-slate-800/40 px-4 py-3">
                            ...
                            <span class="text-sm font-semibold text-emerald-400">{{ today.proxiesAssigned }}</span>
                        </div>
                        <div class="flex items-center justify-between rounded-lg bg-slate-800/40 px-4 py-3">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        >
                            ...
                            <p class="mt-2 text-2xl font-bold" :class="colorClasses[stat.color].text">{{ stat.value }}</p>
                            <p class="mt-1 truncate text-[11px] text-slate-500">{{ stat.sub }}</p>
                            <div class="mt-2 h-1 rounded-full bg-slate-800">
                                <div class="h-1 rounded-full" :class="colorClasses[stat.color].bar" style="width: 60%"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
                        ...
                        <div class="mt-4 space-y-4">
                            <div v-for="item in liveActivity" :key="item.id" class="flex items-start gap-3">
                                <span class="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full" :class="colorClasses[item.color].dot"></span>
            ...
            <!-- Today's absences + quick actions -->
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                <div class="rounded-xl border border-slate-800 bg-slate-900/50 lg:col-span-2">
                    <div class="flex items-center justify-between border-b border-slate-800 px-5 py-4">
                        ...
                        <a href="/proxy-manager" class="flex items-center gap-1 text-sm font-medium text-emerald-400 hover:text-emerald-300">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-sm">
                            ...
                            <tbody class="divide-y divide-slate-800">
                                <tr v-for="row in todaysAbsences" :key="row.teacher">
                                    ...
                                    <td class="px-5 py-3">
                                        <span class="rounded-full px-2.5 py-1 text-xs font-medium" :class="statusBadge[row.status]">
                                            {{ row.status }}
                                        </span>
                                    </td>
                                </tr>
                ...
                <div class="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
                    ...
                    <div class="mt-4 space-y-2">
                        <a
                            ...
                            :key="action.label"
                            :href="action.href"
                            class="flex items-center gap-3 rounded-lg border border-slate-800 px-4 py-3 text-sm font-medium text-slate-300 hover:bg-slate-800/50"
                        >
                            <component :is="quickActionIcons[action.icon]" class="h-4 w-4 text-emerald-400" />
                            {{ action.label }}
                        </a>
                    </div>
                </div>
